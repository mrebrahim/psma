              ...
              <div key={i} className="bg-white border border-[var(--color-line)] rounded-2xl p-5 text-center">
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-light)] text-white mx-auto mb-3 flex items-center justify-center overflow-hidden ring-4 ring-white shadow-md">
                  {m.photo ? (
                    ...
                    <Image src="/images/logo-shield.png" alt="بصمة شباب مصر" width={48} height={48} className="w-12 h-12 object-contain opacity-90" />
                  )}
                </div>
                <div className="font-extrabold text-[var(--color-primary)] text-sm mb-0.5">{m.name}</div>
                <div className="text-xs text-[var(--color-muted)] leading-snug">{m.role}</div>
              </div>
            ))}
          </div>
        </div>
